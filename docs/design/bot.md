...
- `queued` → `thinking` → tool（`recallEpisodes` / `userLookup` は 📝、`webFetch` / `webSearch` は 🔍、`loadSkill` は 📖）→ `thinking` を繰り返す
...
- Discord Gateway listener はローカルの `http://127.0.0.1:${PORT}/webhooks/discord` を webhook URL として渡し、forwarding モードで bot メッセージも受信する
