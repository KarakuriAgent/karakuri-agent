...
3 コマンドのうち post_sns は構造的な動機供給が最も弱い（check_phone は未読件数という外的シグナルがあるが、「伝えたい」は内発）。そこで既存データからの決定論導出で補う: **直近 24h に importance の高い（0.6 以上）エピソードがあり、かつ直近 8h に投稿していない**とき、`<drives>` に「最近、心が動く出来事があった。誰かに話したい気持ちがある。」を注入する。

- importance は appraisal 由来（感情が動いた体験ほど高い）なので、感情の出所は本人の体験 — ツールが内発的感情を捏造しない原則と整合
- 一度投稿すればクールダウン明けまで圧が消える（構造的に連投しない）。8h クールダウンが実質のペースメーカーで、圧の発生は最大 3 回/日
- 内容の重複は既存の 4 層（直近行動ログの提示・話題偏り検出・レート制限・行動の飽き圧）で抑制する
