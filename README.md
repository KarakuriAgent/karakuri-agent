...
- `SNS_*` 設定時は system ユーザー向けにビルトイン SNS skill が追加される。Mastodon と X は `SNS_PROVIDER` で切り替える。cron では `loadSkill("sns")` で `sns_*` ツール群を遅延公開し、SNS 専用ループだけが `autoLoadSnsSkill` と動的コンテキストを使って自動実行する。動的コンテキストには新着通知・トレンド・直近行動ログが含まれ、重複いいね/リポスト/返信/引用をツール層で防ぐ。ループ間隔は `SNS_LOOP_MIN_INTERVAL_MINUTES` / `SNS_LOOP_MAX_INTERVAL_MINUTES` で制御する。heartbeat の活動レポートは `REPORT_CHANNEL_ID` が `postMessage` の送信許可チャンネルにも含まれる構成でのみ案内される。X は `sns_post` の公開範囲が `public` のみ。対話ユーザーに公開する場合は運用側で shared skill を定義する
