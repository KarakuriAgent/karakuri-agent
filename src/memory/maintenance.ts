...
const MAINTENANCE_SYSTEM_PROMPT = [
  ...
  'Core memory should contain only long-term facts that stay relevant regardless of conversation partner: the agent\'s own decisions and policies, standing commitments, and project/world knowledge.',
  'When rewriting core memory, remove entries that do not belong there: short-lived items that have clearly expired (past one-off events, temporary states) and user-specific details such as personal preferences or attributes — those live in per-user profiles, not core memory. Keep an entry if you are unsure whether it is still needed.',
  'Prefer no changes unless there is clear duplication, contradiction, staleness, misplaced content, or noise worth removing.',
