...
function createUserStore(): IUserStore {
  return {
    ...
    ensureUser: vi.fn(async (userId: string, displayName: string) => ({ userId, displayName, createdAt: '', updatedAt: '' })),
...
describe('user alias tools', () => {
  it('exposes linkUser/unlinkUser only to non-KW admins', async () => {
    ...
    const tools = createAgentTools({ userStore, adminUserIds: ['admin'], userId: 'admin' });
    ...
    const kwTools = createAgentTools({ userStore, adminUserIds: ['admin'], userId: 'admin', kwMode: true });
    expect(kwTools.linkUser).toBeUndefined();
  });

  it('does not expose alias tools to non-admin users', () => {
    const tools = createAgentTools({ userStore: createUserStore(), adminUserIds: ['admin'], userId: 'user' });
  ...
  it('does not expose alias tools to the synthetic system user', () => {
    const tools = createAgentTools({ userStore: createUserStore(), adminUserIds: ['admin'], userId: 'system' });
    expect(tools.linkUser).toBeUndefined();
    expect(tools.unlinkUser).toBeUndefined();
  });
