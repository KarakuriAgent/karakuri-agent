...
describe('ElythProvider', () => {
  ...
  it('marks a full notification page complete when sinceId is found', async () => {
    const notifications = ['n5', 'n4', 'n3', 'n2', 'n1'].map((id, index) => ({
      notification_id: id,
      notification_type: 'mention',
      notification_created_at: `2025-01-01T00:00:0${5 - index}.000Z`,
      post_id: `p${id}`,
      post_author_id: `a${id}`,
      post_author_handle: `a${id}`,
    }));
    const fetchMock = vi.fn(async () => Response.json({ notifications }));

    await expect(provider(fetchMock).getNotifications({ limit: 5, sinceId: 'n4' })).resolves.toEqual({
      notifications: [expect.objectContaining({ id: 'n5' })],
      complete: true,
    });
  });
