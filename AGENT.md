...
Discord を主導線にした TypeScript 製の AI エージェント。Vercel AI SDK + Chat SDK + OpenAI 互換 LLM で応答を生成し、ファイルベースのコアメモリ / セッション管理、SQLite による日記・ユーザー・SNS 活動の永続化、Heartbeat / Cron による system turn 実行、Mastodon 系 SNS 連携、Karakuri World 専用モードを備える。
...
- **SNS の重複防止と予約実行**: SNS 活動は SQLite に記録し、like / repost / reply / quote の重複防止と delayed action 実行を行う。
